...
flip        = 18;                                   % degrees
inc         = 2;                                    % degrees
iter        = 50000;                                % Number of Monte-Carlo iterations
SNR         = 20;
NEX = 9;
BinEdges = 30:3:300;
...
[LUTs] = build_LUTs_vdphi(dphi, flip*(pi/180.0), TR, 10.0, T1);
...
for ll = 1:iter
    ...
    % phase difference
    theta(ll) = (theta_pos - theta_neg)/2;
    theta_nex(ll) = (theta_pos_nex - theta_neg_nex)/2;
    
end

%theta = conv(theta, ones(1,9)/9);
T2map = T2map_phase(theta*(pi/180), LUTs);
T2map_nex = T2map_phase(theta_nex*(pi/180), LUTs);
R2map = 1./T2map;
R2map_nex = 1./T2map_nex;

disp(['Mean: ' num2str(1/mean(T2map))]);
if T2 < 10
    save('R2_distribution_low_T2.mat', 'T1', 'T2', 'TR', 'TE', 'flip', 'inc', 'R2map');
    numbin = 75;
else
    save('R2_distribution_high_T2.mat', 'T1', 'T2', 'TR', 'TE', 'flip', 'inc', 'R2map');
    numbin = 10;
end

histogram(1./T2map, BinEdges, 'EdgeColor','none','Normalization','pdf');
histogram(R2map_nex, BinEdges, 'EdgeColor','none','Normalization','pdf');
xlabel('R2 (s^{-1})');
