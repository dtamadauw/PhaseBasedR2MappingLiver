...
T1          = 1000/R1;                              % ms
all_R2      = [30 150];                             % /s
all_T2      = 1000./all_R2;                         % ms
TE          = 0;                                    % ms
TR          = 10;                                   % ms
flip        = 18;                                   % degrees
inc         = 2;                                    % degrees
iter        = 5000;                                % Number of Monte-Carlo iterations
SNR         = 20;
BinEdges = -5:0.3:22;
...
rng(0804);
yfit = {}; ind_yfit = 1;
%% Simulation (histogram of phase distributions in healthy vs iron overload tissue)

figure; hold on;

for T2 = all_T2

% signal at positive and negative rf phase increment
[Msig_pos] = analytical_SPGR(TR*1e-3, T1*1e-3, T2*1e-3, flip*(pi/180.0), dphi/2.0);
[Msig_neg] = analytical_SPGR(TR*1e-3, T1*1e-3, T2*1e-3, flip*(pi/180.0), -dphi/2.0);

Msig_pos_noise_end = zeros(1,iter);
Msig_neg_noise_end = zeros(1,iter);
theta = zeros(1,iter);
theta_nex = zeros(1,iter);

for ll = 1:iter
    %% Signal phase at different RF increments

    % positive RF increment
    noise_std = abs(Msig_pos)/SNR;
    noise_pos = sqrt(noise_std^2/2) * (randn(1,NEX) + 1i*randn(1,NEX));
    Msig_pos_noise = Msig_pos + noise_pos(1);
    Msig_pos_noise_NEX = Msig_pos + mean(noise_pos);
    theta_pos = angle(Msig_pos_noise)*180/pi;
    theta_pos_nex = angle(Msig_pos_noise_NEX)*180/pi;

    % negative RF increment
    noise_std = abs(Msig_neg)/SNR;
    noise_neg = sqrt(noise_std^2/2) * (randn(1,NEX) + 1i*randn(1,NEX));
    Msig_neg_noise = Msig_neg + noise_neg(1);
    Msig_neg_noise_NEX = Msig_neg + mean(noise_neg);
    theta_neg = angle(Msig_neg_noise)*180/pi;
    theta_neg_nex = angle(Msig_neg_noise_NEX)*180/pi;

    %% Phase Estimate

    % phase difference
    all_theta = (theta_pos - theta_neg)/2;
    theta(ll) = all_theta(end);
    theta_nex(ll) = (theta_pos_nex - theta_neg_nex)/2;
    
end

h=histogram(theta, BinEdges, 'EdgeColor','none','Normalization','pdf');
pd = fitdist(theta','Normal');yfit{ind_yfit} = pdf(pd,BinEdges);%;plot(BinEdges,y,'k','LineWidth',2)
histogram(theta_nex, BinEdges, 'EdgeColor','none','Normalization','pdf');
pd = fitdist(theta_nex','Normal');yfit{ind_yfit+1} = pdf(pd,BinEdges);%plot(BinEdges,y,'k','LineWidth',2)
ind_yfit = ind_yfit +2;
xlabel('\theta (\circ)');
    
    if T2 < 10
        theta_end = theta;
        save('phase_distribution_low_T2.mat', 'T1', 'T2', 'TR', 'TE', 'flip', 'inc', 'theta_end');
    else
        theta_end = theta;
        save('phase_distribution_high_T2.mat', 'T1', 'T2', 'TR', 'TE', 'flip', 'inc', 'theta_end');
    end

end

for ii=1:length(yfit)
    plot(BinEdges,yfit{ii},'k','LineWidth',2);
end

ylabel('Probability Density');
legend([ num2str(all_R2(1)) ' s^{-1}'], [num2str(all_R2(1)) ' s^{-1} (Avg=9)'],...
        [num2str(all_R2(2)) ' s^{-1}'], [num2str(all_R2(2)) ' s^{-1} (Avg=9)']); 
