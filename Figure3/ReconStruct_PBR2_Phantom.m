...
%Raw data for phantom at 1.5T
file_1p5T = '../phantom_raw_T2_data/Phantom/raw_data_1p5T/im_data.mat';
%Raw data for phantom at 1.5T
file_3T = '../phantom_raw_T2_data/Phantom/raw_data_3T/im_data.mat';

%Example code for 1.5T data
...
%Define coefficient of relationship between R1 and R2 for phantoms.
...
% Obtain phase
phase_R2 = angle(img_recon(:,:,:,1).*conj(img_recon(:,:,:,2)))/2;
[LUTs] = build_LUTs_vdphi_T1B1(dphi*(pi/180), FA*(pi/180), TR*1000, true, coeff);
[LUTs_WO_R1] = build_LUTs_vdphi_T1B1(dphi*(pi/180), FA*(pi/180), TR*1000, true);

kern = ones(3,3,1)/(3*3*1);

%Convolution in complex image domain
img_recon_conv = convn(img_recon, kern,'same');
ph_conv = angle(img_recon_conv(:,:,:,2).*conj(img_recon_conv(:,:,:,1)))/2.0;


%Calculate T2 map slice-by-slice
T2map = T2map_phase(ph_conv(:,:,1), LUTs);
R2map = 1./T2map;                
figure;
imagesc(R2map); title('R2 map W/ R1 correction')

T2map_WO_R1 = T2map_phase(ph_conv(:,:,1), LUTs_WO_R1);
R2map_WO_R1 = 1./T2map_WO_R1;                
figure;
imagesc(R2map_WO_R1); title('R2 map W/O R1 correction')
